document.addEventListener("DOMContentLoaded", () => {
  ...
  function validateField(input) {
    ...
    if (
      ["first_name", "middle_name", "last_name"].includes(input.name) &&
      value.length > 0
    ) {
    ...
    if (["first_name", "middle_name", "last_name"].includes(input.name)) {
      if (!namePattern.test(value))
        return capitalizeMessage(`${fieldName}: No special characters`);
      if (/^\d+[A-Za-z]/.test(value))
        return capitalizeMessage(`${fieldName}: Numbers first not allowed`);
      if (/\s{2,}/.test(value))
        return capitalizeMessage(`${fieldName}: No double spaces`);
      if (value === value.toUpperCase() && value.length > 1)
        return capitalizeMessage(`${fieldName}: Avoid all caps`);
      if (tripleLetterPattern.test(value.toLowerCase()))
        return capitalizeMessage(`${fieldName}: No 3 same letters in a row`);
      if (value !== capitalizeFirst(value))
        return capitalizeMessage(`${fieldName}: Start with capital letter`);
    }
    ...
    if (input.name === "extension") {
      if (
        value === "Other" &&
        otherExtensionInput &&
        otherExtensionInput.value.trim() !== ""
      ) {
        ...
        if (!validOtherExtensions.includes(otherVal)) {
          return capitalizeMessage(
            `Other Extension: Must be I, II, III, IV, V, IX, or X`
          );
        }
